...
func (h *Handler) GetRegister(c echo.Context) error {
	return Render(c, auth.Show(auth.Data{Type: auth.Register, CSRF: c.Get("csrf").(string)}))
}

func (h *Handler) GetLogin(c echo.Context) error {
	return Render(c, auth.Show(auth.Data{Type: auth.Login, CSRF: c.Get("csrf").(string)}))
}
